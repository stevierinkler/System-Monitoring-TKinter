...
STYLE = {"bg": "#1e1e1e", "fg": "#00ff99", "font": ("Courier", 12, "bold")}
...
def show_cpu():
    ...
    if cpu_window is None:
        ...
        label_cpu = tk.Label(cpu_window,**STYLE)
...
def show_ram():
    ...
    if ram_window is None:
        ...
        label_total_ram = tk.Label(ram_window, **STYLE)
        label_total_ram.place(x=50, y=20)

        label_ram = tk.Label(ram_window, **STYLE)
        label_ram.place(x=50, y=50)

        label_mem_percentage = tk.Label(ram_window, **STYLE)
        label_mem_percentage.place (x=50, y=80)

        label_ram_warning = tk.Label(ram_window, bg="#1e1e1e")
        label_ram_warning.place(x=50, y=110)

    label_total_ram.config(text= f"Total memory: {total_memory:.2f} GB", **STYLE)
    label_ram.config(text= f"Total available memory: {available_mem:.2f} GB",**STYLE)
    label_mem_percentage.config(text= f"Using: {mem.percent:.2f} % of RAM",**STYLE)
...
def show_network():
    ...
    if network_window is None:
        ...
        label_total_send = tk.Label(network_window,**STYLE)
        label_total_send.place(x=50, y=30)

        label_total_received = tk.Label(network_window,**STYLE)
        label_total_received.place(x=50, y= 60)

        label_interval_send = tk.Label(network_window,**STYLE)
        label_interval_send.place(x=50, y=150)

        label_interval_received = tk.Label(network_window,**STYLE)
        label_interval_received.place(x=50, y=120)
    
    label_total_send.config(text= f"Number of total bytes sent : {total_bytes_send:.2f} Mb",**STYLE)
    label_total_received.config(text= f"Number of total bytes received : {total_bytes_recv:.2f} Mb",**STYLE)
    label_interval_received.config(text= f"Number of bytes sent : {interval_bytes_send:.2f} Mb",**STYLE)
    label_interval_send.config(text= f"Number of bytes received : {interval_bytes_recv:.2f} Mb",**STYLE)

    network_window.after(15000, show_network)

# Storage/Disk function 
...
def show_storage():
    ...
    if storage_window is None:
        ...
        label_total_storage = tk.Label(storage_window,**STYLE)
        label_total_storage.place (x=50, y=30)
        label_used_storage = tk.Label(storage_window,**STYLE)
        label_used_storage.place (x=50, y= 70)
        label_percentage_storage_used = tk.Label(storage_window,**STYLE)
    ...
    label_total_storage.config(text= f"total existing storage: {total_storage:.0f} GB",**STYLE)
    label_used_storage.config(text= f"used storage: {used_storage:.0f} GB",**STYLE)
    label_percentage_storage_used.config(text= f"{storage_used_percentage} % of storage is used",**STYLE)
